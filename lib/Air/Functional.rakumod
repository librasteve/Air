...
#| role Tag is used to mark an inner that was generated by Air::Functional
#| and thus to be considered safe for HTML::Escape
role Tag is Str is export(:MANDATORY) {...}

subset Attr   of Str is export(:MANDATORY);
subset Markup of Str is export(:MANDATORY);
subset Inner  where Tag | Markup;
...
sub do-regular-tag($tag, *@inners, *%h --> Markup() ) is export(:MANDATORY)  {
    my $nl = @inners >= 2;
    opener($tag, |%h) ~ inner(@inners) ~ closer($tag, :$nl)
}

#| do a singular tag (ie a tag without @inners)
sub do-singular-tag($tag, *%h --> Markup() ) is export(:MANDATORY)  {
    "\n" ~ '<' ~ $tag ~ attrs(%h) ~ ' />'
}
